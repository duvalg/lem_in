...
/*   Updated: 2017/09/27 17:23:02 by gduval           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		check_room(t_room *room, t_ants *ant, t_dna *dna, int index)
{
	if (room->index == index && index != -1)
		if ((room->active == 0 || !ft_strcmp(room->name, dna->rooms[END].name)))
		{
	...
	while (++add <= 0)
	{
		...
		while (tmp->next)
		{
			if (!ft_strcmp(*ants->room, tmp->link_a))
				if (check_room(find_room_by_name(&tmp->link_b, dna), \
				ants, dna, index + add))
					return (1);
			if (!ft_strcmp(*ants->room, tmp->link_b))
				if (check_room(find_room_by_name(&tmp->link_a, dna), \
				ants, dna, index + add))
					return (1);
			tmp = tmp->next;
		}
